/*
 * 文件名：BucketAggregationExamples.java
 * 修改人：TANCZ1
 * 修改时间：2017年2月13日
 */
...
import com.essentials.elasticsearch.chapter3.*;
...
public class BucketAggregationExamples {
	...
	public static void main(String[] args) throws IOException {
		//Uncomment the below section of code to execute and test all the methods
		 		...
		 		String indexName = "dw_label";
		 		String docType = "dw_label";
		 		String fieldName="label_level";
		//
		...
		//		operations.deleteDocument(indexName, docType);
		 //		operations.searchDocument(indexName, docType);
		 		BucketAggregationExamples bucketaggregation=new BucketAggregationExamples();
		 		bucketaggregation.rangeAggregation(indexName, docType, fieldName);
	...
	private void rangeAggregation(String indexName, String docType,String fieldName) {
		AggregationBuilder aggregation =
				...
				.addRange(1.0f, 2.0f)               // from 1.0 to 100.0 (excluded)
				.addUnboundedFrom(2.0f);            // from 100.0 to +infinity
		SearchResponse response = client.prepareSearch(indexName).setTypes(docType)
				.setQuery(QueryBuilders.matchAllQuery()).addAggregation(aggregation)
				.execute().actionGet();
